...
export async function GET(request: NextRequest) {
  ...
  const [productsResult, batchesResult, movementsResult, allocationsResult, ordersResult] = await Promise.all([
    ...
    db.from("inventory_batches").select("id,product_id,lot_number,manufactured_at,expires_at,received_quantity,remaining_quantity,status,notes,created_at,updated_at").order("created_at", { ascending: true }),
...
type InventoryBody = {
  action?: "receive" | "adjust" | "reassign" | "batchStatus";
  productId?: string;
  lotNumber?: string;
...
export async function POST(request: NextRequest) {
  ...
  if (body.action === "receive") {
    ...
    if (!validDate(body.manufacturedAt) || !validDate(body.expiresAt)) {
      return NextResponse.json({ error: "Проверьте даты партии" }, { status: 400 });
    }
    const { data, error } = await db.rpc("inventory_receive_batch", {
