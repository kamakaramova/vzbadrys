...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await db.from("promo_codes").select("id, code, discount_percent, active, usage_count, max_uses, expires_at, created_at").order("created_at", { ascending: false });
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await db.from("promo_codes").insert({ code, discount_percent: discount, expires_at: expiresAt }).select("id, code, discount_percent, active, usage_count, max_uses, expires_at, created_at").single();
