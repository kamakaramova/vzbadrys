...
type Supply = { id: string; supply_number: string; totalCost: number; extraCosts: number; totalReceived: number; totalRemaining: number; products: Array<{ id: string; name: string; received: number; remaining: number }> };
...
function EmptyPayload(): Payload {
  return { from: monthStart(), to: today(), settings: { usnRateBps: 600 }, summary: { paidOrders: 0, revenue: 0, deliveryCollected: 0, deliveryActual: 0, deliveryDifference: 0, cogs: 0, operatingExpenses: 0, profitBeforeTax: 0, estimatedTax: 0, actualTax: 0, estimatedNetProfit: 0, uncostedUnits: 0 }, daily: [], expenses: [], batches: [], supplies: [] };
}
  ...
  return <section className="space-y-5 min-w-0">
    ...
    {loading ? <div className="py-24 flex justify-center text-[#999]"><LoaderCircle className="animate-spin"/></div> : <>
      ...
      <div className="grid xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,.65fr)] gap-4">
        ...
        <div className="rounded-2xl border border-[#eadfd8] bg-white overflow-hidden"><div className="px-4 py-3 border-b border-[#eee7e2]"><h3 className="font-bold">Себестоимость поставок</h3><p className="text-xs text-[#999] mt-0.5">Одна общая сумма поставки для всех добавок, принятых вместе.</p></div><div className="max-h-[440px] overflow-auto divide-y divide-[#f0e8e2]">{data.supplies.map((supply) => <div key={supply.id} className="px-4 py-3"><div className="flex justify-between gap-3"><b className="font-mono text-sm">{supply.supply_number}</b><b className="tabular-nums">{money(supply.totalCost)}</b></div><div className="mt-1 flex justify-between text-xs text-[#837a75]"><span>{supply.totalRemaining} из {supply.totalReceived} осталось</span><span>{supply.totalReceived ? `${money(Math.round(supply.totalCost / supply.totalReceived))} / ед.` : "—"}</span></div><div className="mt-1.5 space-y-0.5 text-[11px] text-[#837a75]">{supply.products.map((product) => <div key={product.id}>{product.name}: {product.remaining} из {product.received} осталось</div>)}</div>{supply.extraCosts > 0 && <div className="mt-1 text-[11px] text-[#b85d3b]">Доп. расходы поставки: {money(supply.extraCosts)}</div>}</div>)}{!data.supplies.length && <div className="px-4 py-10 text-center text-sm text-[#aaa]">Общих поставок пока нет.</div>}</div></div>
      </div>
    </>}
