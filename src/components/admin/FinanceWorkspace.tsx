...
type Expense = { id: string; occurred_on: string; period_from: string | null; period_to: string | null; amount_kopecks: number; category: string; description: string | null; batch_id: string | null; supply_id: string | null; batch: { id: string; lotNumber: string } | null; supply: { id: string; supplyNumber: string } | null };
...
type Supply = { id: string; supply_number: string; directCosts: number; allocatedCommonCosts: number; totalCost: number; totalReceived: number; totalRemaining: number; soldRevenue: number; soldUnits: number; soldOrders: number; averageOrderRevenue: number; projectedOrders: number; projectedRevenue: number; projectedDelivery: number; projectedNetProfit: number; estimatedCostPerOrder: number; products: Array<{ id: string; name: string; received: number; remaining: number }> };
type Daily = { date: string; revenue: number; deliveryActual: number; cogs: number; operating: number; profitBeforeTax: number };
type Payload = { from: string; to: string; selectedSupplyId: string; settings: { usnRateBps: number }; summary: { paidOrders: number; revenue: number; deliveryCollected: number; deliveryActual: number; deliveryDifference: number; cogs: number; totalPeriodExpenses: number; directPeriodSupplyExpenses: number; generalPeriodExpenses: number; profitBeforeTax: number; estimatedTax: number; actualTax: number; estimatedNetProfit: number; uncostedUnits: number }; daily: Daily[]; expenses: Expense[]; batches: Batch[]; supplies: Supply[] };
type ExpenseDraft = { id?: string; occurredOn: string; amount: string; category: string; batchId: string; supplyId: string; periodFrom: string; periodTo: string; description: string };
...
function EmptyPayload(): Payload {
  return { from: monthStart(), to: today(), selectedSupplyId: "", settings: { usnRateBps: 600 }, summary: { paidOrders: 0, revenue: 0, deliveryCollected: 0, deliveryActual: 0, deliveryDifference: 0, cogs: 0, totalPeriodExpenses: 0, directPeriodSupplyExpenses: 0, generalPeriodExpenses: 0, profitBeforeTax: 0, estimatedTax: 0, actualTax: 0, estimatedNetProfit: 0, uncostedUnits: 0 }, daily: [], expenses: [], batches: [], supplies: [] };
}
...
export default function FinanceWorkspace({ password }: { password: string }) {
  ...
  const [draft, setDraft] = useState<ExpenseDraft>({ occurredOn: today(), amount: "", category: "other", batchId: "", supplyId: "", periodFrom: "", periodTo: "", description: "" });
  const [rate, setRate] = useState("6");

  const load = async (quiet = false, periodFrom = from, periodTo = to, supplyId = selectedSupplyId) => {
    if (!quiet) setLoading(true); setError("");
    try {
      const response = await fetch(`/api/admin/finance?from=${periodFrom}&to=${periodTo}&supply=${encodeURIComponent(supplyId)}`, { headers: { "x-admin-password": password }, cache: "no-store" });
  ...
  const openNew = () => { setDraft({ occurredOn: today(), amount: "", category: "other", batchId: "", supplyId: "", periodFrom: "", periodTo: "", description: "" }); setShowForm(true); };
  const openEdit = (expense: Expense) => { setDraft({ id: expense.id, occurredOn: expense.occurred_on, amount: String(expense.amount_kopecks / 100), category: expense.category, batchId: expense.batch_id || "", supplyId: expense.supply_id || "", periodFrom: expense.period_from || "", periodTo: expense.period_to || "", description: expense.description || "" }); setShowForm(true); };
  const saveExpense = async () => {
    const ok = await post({ action: "saveExpense", id: draft.id, occurredOn: draft.occurredOn, amountKopecks: Math.round(Number(draft.amount) * 100), category: draft.category, batchId: draft.batchId || null, supplyId: draft.supplyId || null, periodFrom: draft.periodFrom || null, periodTo: draft.periodTo || null, description: draft.description }, draft.id ? "Расход обновлён." : "Расход добавлен в учёт.");
  ...
  const metrics = [
    ...
    ["Себестоимость товаров в заказах", money(data.summary.cogs), data.summary.uncostedUnits ? `Не распределено по партиям: ${data.summary.uncostedUnits} шт.` : "Справочно: доля затрат поставок в уже проданных товарах", data.summary.uncostedUnits ? "text-amber-700" : "text-[#222]"],
    ["Все расходы периода", money(data.summary.totalPeriodExpenses), `Поставка: ${money(data.summary.directPeriodSupplyExpenses)} · общие: ${money(data.summary.generalPeriodExpenses)}`, "text-[#222]"],
  ...
  return <section className="space-y-5 min-w-0">
    ...
    <div className="rounded-2xl border border-[#eadfd8] bg-white p-3 flex flex-wrap items-end gap-3">
      ...
      <label className="text-xs font-semibold text-[#756c67]">Поставка<select value={selectedSupplyId} onChange={(event) => setSelectedSupplyId(event.target.value)} className="block mt-1 min-w-[210px] rounded-xl border border-[#eadfd8] bg-white px-3 py-2 text-sm"><option value="">Все поставки</option>{data.supplies.map((supply) => <option key={supply.id} value={supply.id}>{supply.supply_number}</option>)}</select></label>
